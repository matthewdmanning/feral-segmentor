...
def _load_annotation(path: Path) -> Annotation:
    ...
    path : Path
        ...
        ``.png``/``.jpg``/``.bmp`` → :class:`~feral_segmentor.data.annotations.MaskAnnotation`;
        ``.txt`` → :class:`~feral_segmentor.data.annotations.BBoxAnnotation`.
...
class DatasetSource:
    ...
    :class:`~feral_segmentor.data.dataset.AnnotationDataset` or
    :class:`~feral_segmentor.data.dataset.StreamingAnnotationDataset`.
