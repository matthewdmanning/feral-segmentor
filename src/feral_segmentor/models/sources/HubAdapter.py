...
Training workflow  : HubAdapter().fetch(cfg) → HubAdapter().instantiate(cfg)
Registry workflow  : HubAdapter().inspect(cfg)  [via register_model.load_properties]
"""
...
def _inspect_loaded(model: nn.Module) -> ModelProperties:
    """Infer model_outputs from the last Linear or Conv2d layer of a loaded model."""
